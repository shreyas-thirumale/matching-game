...
*/
function initGame() {
    // Reset the game board by clearing any existing HTML
    const gameBoard = document.getElementById('game-board');
    gameBoard.innerHTML = '';

    // Create cards array with two of each symbol
    cards = [...symbols, ...symbols];

    // Shuffle the cards
    shuffleArray(cards);

    // Create and add each card to the game board
...
function createCard(symbol) {
    ...
    card.dataset.symbol = symbol; // Save the symbol in the element
...
function checkForMatch() {
    ...
    if (firstCard.dataset.symbol === secondCard.dataset.symbol) {
        // It's a match! Disable the cards
        disableCards();
    } else {
...
function disableCards() {
    // Add matched class to both cards to disable them
